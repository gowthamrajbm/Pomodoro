...
class UserInterface:
    def __init__(self, master):
        ...
        self._window_width = 200
        self._window_height = 215
        self._window_offset_x = 220
        self._window_offset_y = 270
        self._last_screen_width = master.winfo_screenwidth()
        self._last_screen_height = master.winfo_screenheight()
        self._set_window_position()
        self.master.attributes("-topmost", True)
        self.master.configure(bg="#111111")
        self._monitor_screen_resolution()
    def _set_window_position(self):
        screen_width = self.master.winfo_screenwidth()
        screen_height = self.master.winfo_screenheight()
        x = screen_width - self._window_offset_x
        y = screen_height - self._window_offset_y
        self.master.geometry(f"{self._window_width}x{self._window_height}+{x}+{y}")

    def _monitor_screen_resolution(self):
        def check_resolution():
            screen_width = self.master.winfo_screenwidth()
            screen_height = self.master.winfo_screenheight()
            if screen_width != self._last_screen_width or screen_height != self._last_screen_height:
                self._last_screen_width = screen_width
                self._last_screen_height = screen_height
                self._set_window_position()
            self.master.after(1000, check_resolution)
        check_resolution()
        ...
        self.close_button = Button(self.master, text="✕", command=self.master.destroy,
        ...
        self.close_button.place(relx=1.0, x=-4, y=4, anchor="ne", width=16, height=16)
    ...
    def open_settings(self):
        ...
        # Disable the close button on the master window
        ...
        def set_popup_position():
            screen_width = settings_popup.winfo_screenwidth()
            screen_height = settings_popup.winfo_screenheight()
            x = screen_width - 320
            y = screen_height - 630
            settings_popup.geometry(f"{popup_width}x{popup_height}+{x}+{y}")
        set_popup_position()
        settings_popup.configure(bg="#222222")
        def monitor_popup_resolution():
            screen_width = settings_popup.winfo_screenwidth()
            screen_height = settings_popup.winfo_screenheight()
            if (screen_width != monitor_popup_resolution.last_width or screen_height != monitor_popup_resolution.last_height):
                monitor_popup_resolution.last_width = screen_width
                monitor_popup_resolution.last_height = screen_height
                set_popup_position()
            settings_popup.after(1000, monitor_popup_resolution)
        monitor_popup_resolution.last_width = settings_popup.winfo_screenwidth()
        monitor_popup_resolution.last_height = settings_popup.winfo_screenheight()
        monitor_popup_resolution()
        # Add orange border using a Frame
        ...
        def open_goals_file():
            ...
            if save_dir:
                ...
                if os.path.isfile(file_path):
                    ...
                    mb.showinfo("File Not Found", f"{file_path} does not exist. Create your first goal to generate it.")
            else:
                import tkinter.messagebox as mb
...
def main():
    ...
    lockfile = os.path.join(tempfile.gettempdir(), "pomodoro_app.lock")
    try:
        if os.path.exists(lockfile):
            import tkinter.messagebox as mb
            mb.showerror("Already Running", "Pomodoro app is already running.")
            return
